<!--
  CANONICAL FLOCKION RULESET — single source of truth.
  Edit this file, then run `npm run build:adapters` to regenerate every
  harness adapter (AGENTS.md, .cursor, .windsurf, .clinerules, .github, .kiro).
  `npm run check` fails if any adapter has drifted from this file.
  Do NOT hand-edit the generated adapters.
-->
